...
<body class="flex flex-col h-screen">
    ...
    <!-- Navbar start -->
    <nav class="navbar bg-gray-800 flex-initial">
    ...
    ?>
    <div class="transactions flex-auto font-bold">
        <h1 class="text-4xl text-center my-2 underline" style="color: navy;">Transactions History</h1>
        <table class="border-collapse border-2 table-auto w-10/12 mx-auto text-center">
            <thead class="text-2xl bg-yellow-200" style="color: darkblue;">
                <tr>
                    <th class="w-1/9 border-2 th">Transaction ID</th>
                    <th class="w-1/3 border-2 th">Transferred From</th>
                    <th class="w-1/3 border-2 th">Transferred To</th>
                    <th class="w-2/9 border-2 th">Transferred Amount</th>
                </tr>
            </thead>
            <tbody class="text-xl bg-red-100" style="color: blue;">
                ...
                if (mysqli_num_rows($result) > 0) {
                    while($row = mysqli_fetch_assoc($result)) {
                      echo '
                      <tr>
                          <td class="border-2 td">'.$row["id"].'</td>
                          <td class="border-2 td">'.$row["transfer_from"].'</td>
                          <td class="border-2 td">'.$row["transfer_to"].'</td>
                          <td class="border-2 td">&#8377; '.$row["amount"].'.00</td>
                      </tr>
                      ';
                    }
                  } else {
